...
public class CollaborativeFiltering {
	...
	public static String dbpediaIds = "files/ids2dbpedia.dat";
	...
	public static void main(String[] args) throws IOException, TasteException {
		...
		// for all movies
		Set<Long> movieSet = dbpedia.keySet();
		int count = 0;
		for (java.util.Iterator<Long> it = movieSet.iterator(); it.hasNext();) {
			...
			try {
				...
				// prepare File
				FileWriter filewriter = new FileWriter("files/nearestneighbordbpeida.dat", true);
				...
				String uri = dbpedia.get(movieID);
				...
				for (long l : neighbors) {
					...
					try {
						...
						// müssen
						if (dbpedia.get(l) != null) {
							URI object = f.createURI(dbpedia.get(l));
