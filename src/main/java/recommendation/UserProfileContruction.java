...
public class UserProfileContruction {
	 ...
	 * @return 
	 */
	public static HashMap<String, Double> approach3(Vector<Rating> backData, Object ifM , int weighted, HashMap<String, String> idtoURIHashMap) {
		...
		if (backData != null) {
			...
			for (int i = 0; i < backData.size(); i++) {
				...
				if (weighted == Mediator.WEIGHTED) {
					featureVector = ((WeightedItemFeature)ifM).getFeatureOfMovie(idtoURIHashMap.get(r.getMovie_lensID()));
				} else if (weighted == Mediator.UNWEIGHTED) {
					featureVector = ((UnweightedItemFeature)ifM).getFeatureOfMovie(idtoURIHashMap.get(r.getMovie_lensID()));
				}
				// feature vecot kann null sein, wenn feature score für diesen Film nicht gefunden werden konnten
